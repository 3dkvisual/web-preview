...
const translations = {
  es: {
    ...
    'academy.title': 'Dominá CGI, 3D e IA<br>con quienes lo hacen para marcas globales.',
    'academy.sub': 'Workflows reales. Clientes reales. Resultados reales.<br>Las herramientas que usamos en cada proyecto, ahora las enseñamos.',
    ...
    'about.p1': '3DK Visual es un estudio CGI de Buenos Aires especializado en FOOH, DOOH y animación 3D para marcas globales. <strong>+1.000 proyectos</strong> y <strong>+100 clientes</strong> en publicidad, motion y branded content.',
    'about.p2': 'Socios de <strong>Publicis Group</strong> y <strong>WPP · Ogilvy</strong>.',
  ...
  en: {
    ...
    'academy.tag': '3DK Academia',
    'academy.title': 'Master CGI, 3D & AI<br>with those who do it for global brands.',
    ...
    'about.p1': '3DK Visual is a CGI studio from Buenos Aires specialized in FOOH, DOOH and 3D animation for global brands. <strong>1,000+ projects</strong> and <strong>100+ clients</strong> in advertising, motion and branded content.',
    'about.p2': 'Partners of <strong>Publicis Group</strong> and <strong>WPP · Ogilvy</strong>.',
...
const heroClips = ['clip01', 'clip02', 'clip03', 'clip04'];
let heroClipIdx = 0;
let heroTransitioning = false;

function heroSrc(idx) {
  const v = window.innerWidth < 768 ? '-v' : '';
  return `videos/hero/${heroClips[idx]}${v}.mp4`;
}

function applyHeroClipFraming(video, idx) {
  video.classList.toggle('hero-video-contain', heroClips[idx] === 'clip04');
}
...
if (vidA && vidB) {
  ...
  function onHeroTimeUpdate() {
    ...
    if (active.currentTime < active.duration - 0.8) return;
    heroTransitioning = true;

    heroClipIdx = (heroClipIdx + 1) % heroClips.length;
    next.src = heroSrc(heroClipIdx);
    applyHeroClipFraming(next, heroClipIdx);
    ...
    next.play().then(() => {
      ...
      active.style.transition = 'opacity 0.8s ease';
      ...
      setTimeout(() => {
        ...
        next.pause();
        next.src = '';
        next.classList.remove('hero-video-contain');
        next.style.transition = 'none';
        heroTransitioning = false;
      }, 850);
    }).catch(() => { heroTransitioning = false; });
  }

  vidA.addEventListener('timeupdate', onHeroTimeUpdate);
  vidB.addEventListener('timeupdate', onHeroTimeUpdate);
}
